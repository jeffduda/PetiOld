...
void
CommandLineParser
::PrintMenu( std::ostream& os, Indent indent, bool printShortVersion,
  OptionType::Pointer selectedOption ) const
{
  bool printEntireMenu = ( selectedOption.GetPointer() == NULL );
  ...
  for( it = this->m_Options.begin(); it != this->m_Options.end(); it++ )
    {
    if( !printEntireMenu && *it != selectedOption )
						{
      continue;
						}
